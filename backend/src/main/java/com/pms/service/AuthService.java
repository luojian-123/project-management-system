...
public class AuthService {
    ...
    public AuthService(SysUserRepository userRepository, SysMenuRepository menuRepository, SysRoleRepository roleRepository,
    ...
    public Map<String, Object> login(String username, String password) {
        SysUser user = userRepository.selectByUsername(username);
        ...
        List<SysMenu> menus = menuRepository.selectByUserId(user.getId());
        ...
        List<String> roleCodes = roleRepository.selectCodesByUserId(user.getId());
    ...
    public Map<String, Object> getInfo(Long userId) {
        ...
        List<SysMenu> menus = menuRepository.selectByUserId(userId);
        List<Map<String, Object>> menuList = buildMenuTree(menus, 0L);
        Map<String, Object> result = new HashMap<>();
        result.put("userId", user.getId());
        result.put("username", user.getUsername());
        result.put("realName", user.getRealName() != null ? user.getRealName() : user.getUsername());
        result.put("menus", menuList);
        List<String> roleCodes = roleRepository.selectCodesByUserId(userId);
        result.put("roleCodes", roleCodes != null ? roleCodes : List.of());
        return result;
    }
